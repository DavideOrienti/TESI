...
# alpha = peso CF dentro l'ibrido CF+Content
ALPHAS = [0.2, 0.4, 0.5, 0.6, 0.8]

# beta = peso Hybrid rispetto a Popularity
...
OUTPUT_DIR_NAME = "hybrid_with_popularity_v1"
...
def build_popularity_scores(train: pd.DataFrame, min_votes: int) -> dict[int, float]:
    item_stats = (
    ...
    return {
        int(row.movieId): float(row.pop_score)
...
def build_user_model_scores(
    ...
    for movie_id in candidate_items:
        ...
        pop_scores[movie_id] = popularity_scores.get(movie_id, 0.0)

    return cf_scores, content_scores, pop_scores


def recommend_top_k_hybrid_popularity(
    ...
    # normalizzazione per utente
    cf_norm = minmax_normalize_scores(cf_scores)
    content_norm = minmax_normalize_scores(content_scores)
    pop_norm = minmax_normalize_scores(pop_scores)

    all_items = set(cf_norm.keys()).union(content_norm.keys()).union(pop_norm.keys())

    final_scores = []
    for movie_id in all_items:
        scf = cf_norm.get(movie_id, 0.0)
        scontent = content_norm.get(movie_id, 0.0)
        spop = pop_norm.get(movie_id, 0.0)

        hybrid_score = alpha * scf + (1.0 - alpha) * scontent
        final_score = beta * hybrid_score + (1.0 - beta) * spop
...
def evaluate_split_for_params(
    ...
    for _, row in eval_df.iterrows():
        ...
        cf_scores, content_scores, pop_scores = build_user_model_scores(
            user_id=user_id,
        ...
        recs = recommend_top_k_hybrid_popularity(
            cf_scores=cf_scores,
...
def main():
    ...
    popularity_scores = build_popularity_scores(train, POPULARITY_MIN_VOTES)
    ...
    for alpha in ALPHAS:
        for beta in BETAS:
            ...
            val_summary, val_per_user = evaluate_split_for_params(
                train=train,
                eval_df=val,
                candidate_items=candidate_items,
                cf_neighbors_dict=cf_neighbors_dict,
            ...
            test_summary, test_per_user = evaluate_split_for_params(
                train=train,
                eval_df=test,
                candidate_items=candidate_items,
                cf_neighbors_dict=cf_neighbors_dict,
            ...
            val_per_user.to_csv(output_dir / f"val_per_user_alpha_{alpha:.2f}_beta_{beta:.2f}.csv", index=False)
            test_per_user.to_csv(output_dir / f"test_per_user_alpha_{alpha:.2f}_beta_{beta:.2f}.csv", index=False)
    ...
    summary = {
        "config": {
            ...
            "betas": BETAS,
            "top_k_list": TOP_K_LIST,
            "model": "hybrid_with_popularity_v1",
        ...
        "best_params_by_val_ndcg10": {
            "alpha": best_params[0] if best_params else None,
            "beta": best_params[1] if best_params else None
        },
    ...
    if best_params is not None:
        print(f"best alpha={best_params[0]} | best beta={best_params[1]}")
